...
export default function PaymentFlow() {
  ...
  const [amount, setAmount] = useState<string>('3700');
  const [method, setMethod] = useState<Method>('wallet');
  const [cards, setCards] = useState<any[]>([]);
  const [paidVisible, setPaidVisible] = useState(false);
  const token = undefined; // plug in auth token from your app
  ...
  const pay = async () => {
    try {
      ...
      await PaymentsApi.serviceCharge({ rideId: 1, amount: naira, method }, token);
      setPaidVisible(true);
    } catch (e: any) {
    ...
    <View className="px-5 pt-4 pb-6">
      <View className="mt-6">
        ...
        <View className="mt-8 border-t border-dashed border-gray-300 pt-6">
          <Text className="text-center text-gray-700">Pay Service Charge to Rider</Text>
        ...
        <View className="mt-10">
          <Text className="text-lg font-semibold text-primary mb-4">Service Charge Payment</Text>
      ...
      <SuccessModal
        visible={paidVisible}
        title="Paid"
        message={'Thank you for using Izee Ride.\n\nYour Payment has been debited and paid to the Rider.'}
